...
public class ItensZeradosController {
    ...
    public ObservableList<ProdutoMySQL> getProdutos() {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM produtos WHERE estoque = 0");
    ...
    public ObservableList<ProdutoMySQL> getProdutosFiltrados(String filtro) {
        ...
        try {
            Connection conn = conexao.ConnectDB();
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM produtos WHERE id_fornecedor LIKE ? AND estoque = 0");
